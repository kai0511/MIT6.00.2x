...
def r_squared(y, estimated):
    ...
    """
    import numpy as np
    return (1- sum((np.asarray(y) - np.asarray(estimated))**2) / sum((np.asarray(y) - np.mean(y))**2))
...
def evaluate_models_on_training(x, y, models):
    ...
    for m in models:
        deg = len(m) - 1
        p = np.poly1d(m)
        estimated = [p(i) for i in x]
        rSquare = r_squared(y, estimated)
        
        pylab.plot(x, estimated, 'r-', 
                   label = str(deg) + '-degree, R2 = ' + str(round(rSquare,5)))
    pylab.legend(loc = 'best')
    pylab.title('polyfit curve')
    pylab.show()
